    ...
    TimeProvider timeProvider)
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            return IssueAttachmentMutationResult.Failure(IssueAttachmentOutcome.Forbidden);
        }

        var issue = await dbContext.Issues
            .Include(item => item.Project)
            .SingleOrDefaultAsync(item => item.ProjectId == projectId && item.Id == issueId, cancellationToken);
        if (issue is null) return IssueAttachmentMutationResult.Failure(IssueAttachmentOutcome.NotFound);
        if (issue.Project.Status == ProjectStatus.Inactive) return IssueAttachmentMutationResult.Failure(IssueAttachmentOutcome.ProjectInactive);
        if (fileSize <= 0 || string.IsNullOrWhiteSpace(fileName)) return IssueAttachmentMutationResult.Failure(IssueAttachmentOutcome.InvalidFile);
        if (fileSize > options.MaxFileSizeBytes) return IssueAttachmentMutationResult.Failure(IssueAttachmentOutcome.FileTooLarge);
        ...
        {
            return IssueAttachmentMutationResult.Failure(IssueAttachmentOutcome.FileTooLarge);
        }
        ...
        return IssueAttachmentMutationResult.Uploaded(
        ...
        CancellationToken cancellationToken)
    {
        if (!await HasPermissionAsync(projectId, accountId, ProjectManagementConstants.IssueReadPermission, cancellationToken))
        {
            return IssueAttachmentContentResult.Failure(IssueAttachmentOutcome.Forbidden);
        }
        ...
        var attachment = await dbContext.IssueAttachments.AsNoTracking()
            .SingleOrDefaultAsync(
                ...
                cancellationToken);
        if (attachment is null) return IssueAttachmentContentResult.Failure(IssueAttachmentOutcome.NotFound);
        if (!string.Equals(attachment.StorageProvider, fileStorage.Provider, StringComparison.OrdinalIgnoreCase))
        {
            return IssueAttachmentContentResult.Failure(IssueAttachmentOutcome.StorageUnavailable);
        }

        var stream = await fileStorage.OpenReadAsync(attachment.StorageKey, cancellationToken);
        return stream is null
            ? IssueAttachmentContentResult.Failure(IssueAttachmentOutcome.StorageUnavailable)
            : IssueAttachmentContentResult.Success(
                stream,
        ...
        CancellationToken cancellationToken)
    {
        ...
        var attachment = await dbContext.IssueAttachments
            ...
            .SingleOrDefaultAsync(
                ...
                cancellationToken);
        if (attachment is null || attachment.IsDeleted) return IssueAttachmentMutationResult.Failure(IssueAttachmentOutcome.NotFound);
        if (attachment.Issue.Project.Status == ProjectStatus.Inactive) return IssueAttachmentMutationResult.Failure(IssueAttachmentOutcome.ProjectInactive);

        attachment.MarkDeleted(new ChangeContext(accountId, timeProvider.GetUtcNow()));
        await dbContext.SaveChangesAsync(cancellationToken);
        return IssueAttachmentMutationResult.Deleted();
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        return attachments.Select(item => new IssueAttachmentResponse
        {
            ...
            UploadedByUsername = accounts.TryGetValue(item.UploadedByAccountId, out var account)
                ? account.Username
                : string.Empty,
