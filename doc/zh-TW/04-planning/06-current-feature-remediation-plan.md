...
本文件記錄 2026-08-02 對目前 `main` 功能的盤點結果，以及下一輪修補工作的交付順序。它管理執行進度，不取代既有功能規格、資料模型或 HTTP 合約。
...
| 編號 | 類別 | 問題 | 影響 | 優先度 |
| --- | --- | --- | --- | --- |
| R-01 | i18n | 深層專案與測試頁面仍有硬編碼中文；部分共用元件已完成翻譯。 | 英文介面不完整，容易再次出現翻譯鍵值。 | P0 |
| R-02 | 文件 | `05-test-management-implementation-plan.md` 的完成狀態與目前功能不完全一致。 | 文件無法可靠反映已交付範圍。 | P0 |
| R-03 | 文件／規格 | Test Run「取消後可重新開始」已由程式實作，但資料模型、實作計畫與 HTTP 合約仍殘留舊規則；Test Case 對外編號目前尚未實作，需另列為功能需求。 | 規格、資料模型、OpenAPI 與實作可能互相矛盾。 | P0 |
| R-04 | 測試案例 | 缺少案例清單的完整搜尋、篩選與移動操作；Suite 編輯入口亦需補足。 | 測試資產增加後不易維護。 | P1 |
| R-05 | 測試計畫 | Plan 的瀏覽、篩選、修改歷程與操作驗證仍需加強。 | 使用者難以管理大量計畫。 | P1 |
| R-06 | 品質 | 核心流程缺少足夠的 API 與瀏覽器端 E2E 回歸保護。 | 調整結果連動或狀態機時容易回歸。 | P1 |
...
- 將 Test Case 對外編號列為待實作功能，並確認其格式、Prefix 設定位置、唯一性範圍與舊資料補齊方式。
- 依確認後的編號決策更新權威規格與資料模型，再實作 schema、API、前端與資料遷移。
...
### 第四階段：Test Plan 與 Test Run 回歸保護（P1）

- 補齊 Plan／Run 清單的篩選、狀態顯示與空白狀態。
- 為開始、取消、重新開始、完成與案例／步驟結果連動新增 API 測試。
- 為建立 Case、建立 Plan、建立與完成 Run 建立最小 E2E 路徑。
